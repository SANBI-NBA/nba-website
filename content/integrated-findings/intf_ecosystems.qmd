...
#| label: fig-rle-plots
#| fig-cap: Number of ecosystem types in each threat status category in each realm (A); the percentage extent of each ecosystem in each threat status category (B) (note for that length is used for rivers, while area is used for all other realms). In addition to the realms, the plot also shows results for the coastal zone, made up of estuaries and selected marine and terrestrial types, and the subantarctic territory of South Africa.
...
![Spatial distribution of threatened ecosystems for each realm: (A) terrestrial and marine benthic; (B) estuarine and marine pelagic; (C) river; (D) wetland; (E) subantarctic territory including the Prince Edward Islands and surrounding seas. Note: sstuaries, which are not visible at this map scale are shown as circles in panel B, scaled to the size of the estuary](imgs/ets-map8.png){#fig-rle-map width="800"}
