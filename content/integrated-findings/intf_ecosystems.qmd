...
![Percentage of ecosystem types in each protection level category in each realm (A), the percentage extent in each category in each realm (B) (for rivers length is used as the unit of extent, for all other realms area is used). In addition to the realms the plot also shows results for the coastal zone, made up of estuaries and selected marine and terrestrial types, and the subantarctic territory of South Africa.](imgs/epl_combined_plot.png){#fig-epl-plot}
