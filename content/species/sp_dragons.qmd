...
title: "Dragonflies & Damselflies" # Compulsory
...
# ---- odonata ----
df_odonata <- df %>%
  filter(`Taxonomic grouping` == "Dragonflies & Damselflies") %>%
  collapse_rare() %>%
  mutate(Status = factor(Status, levels = status_levels))

df_odonata_en <- df %>%
  filter(`Taxonomic grouping` == "Dragonflies & Damselflies", Endemism == "Endemic") %>%
  collapse_rare() %>%
  mutate(Status = factor(Status, levels = status_levels))

df_odonata_wide    <- make_wide(df_odonata, "Overall", all_levels)
df_odonata_en_wide <- make_wide(df_odonata_en, "Endemic", all_levels)

df_combined <- bind_rows(df_odonata_wide, df_odonata_en_wide)
...
dat1_sel$Taxon <- "Overall dragonflies & damselflies"
dat2_sel$Taxon <- "Endemic dragonflies & damselflies"
...
df_odonata <- df_rlis%>%
  filter(Taxon %in% c("Dragonflies & Damselflies", "Aggregate"))
...
p <- nba_pressure_bar_plot(df, TAXON = "Dragonflies & Damselflies")
p + scale_x_discrete(
  labels = function(x) {
