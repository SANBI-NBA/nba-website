...
title: "Sharks (incl. Batoids & Chimaeras)" # Compulsory
...
# ---- sharks ----
df_sharks <- df %>%
  filter(`Taxonomic grouping` == "Sharks (incl. Batoids & Chimaeras)") %>%
  collapse_rare() %>%
  mutate(Status = factor(Status, levels = status_levels))

df_sharks_en <- df %>%
  filter(`Taxonomic grouping` == "Sharks (incl. Batoids & Chimaeras)", Endemism == "Endemic") %>%
  collapse_rare() %>%
  mutate(Status = factor(Status, levels = status_levels))
...
nba_pressure_bar_plot(
  df,
  TAXON = "Sharks (incl Batoids & Chimaeras)")
```
