...
# ---- reptiles ----
df_reptiles <- df %>%
  filter(`Taxonomic grouping` == "Reptiles") %>%
  collapse_rare() %>%
  mutate(Status = factor(Status, levels = status_levels))

df_reptiles_en <- df %>%
  filter(`Taxonomic grouping` == "Reptiles", Endemism == "Endemic") %>%
  collapse_rare() %>%
  mutate(Status = factor(Status, levels = status_levels))

df_reptiles_wide    <- make_wide(df_reptiles, "Overall", all_levels)
df_reptiles_en_wide <- make_wide(df_reptiles_en, "Endemic", all_levels)

df_combined <- bind_rows(df_reptiles_wide, df_reptiles_en_wide)
...
df_reptiles <- df_rlis%>%
  filter(Taxon %in% c("Reptiles", "Aggregate"))
...
p <- nba_pressure_bar_plot(df, TAXON = "Reptiles")
p + scale_x_discrete(
  labels = function(x) {
