...
# ---- butterflies ----
df_butterflies <- df %>%
  filter(`Taxonomic grouping` == "Butterflies") %>%
  collapse_rare() %>%
  mutate(Status = factor(Status, levels = status_levels))

df_butterflies_en <- df %>%
  filter(`Taxonomic grouping` == "Butterflies", Endemism == "Endemic") %>%
  collapse_rare() %>%
  mutate(Status = factor(Status, levels = status_levels))

df_butterflies_wide    <- make_wide(df_butterflies, "Overall", all_levels)
df_butterflies_en_wide <- make_wide(df_butterflies_en, "Endemic", all_levels)

df_combined <- bind_rows(df_butterflies_wide, df_butterflies_en_wide)
...
df_butterflies <- df_rlis%>%
  filter(Taxon %in% c("Butterflies", "Aggregate"))
...
p <- nba_pressure_bar_plot(df, TAXON = "Butterflies")
p + scale_x_discrete(
  labels = function(x) {
...
dat1 <- read_excel("data/nba_example_pro_data.xlsx", 
                   sheet = "2025")
dat1 <- dat1 %>%
  filter(`Taxon groups`== "Butterflies")
