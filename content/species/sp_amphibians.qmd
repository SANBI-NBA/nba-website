...
google-scholar: true
image: "./imgs/sp_amphibian_img_1.png"
---
...
#| fig-cap: "Spatial distribution of threatened amphibians. The legend reflects the number of threatened plants per 10 x 10 km grid cell. Darker shading shows areas where threatened amphibians are concentrated."
#| out-width: 650px
...
dat1_sel$Taxon <- "Overall amphibians"
dat2_sel$Taxon <- "Endemic amphibians"
...
```{r}
#| label: fig-3
#| fig-cap: "The Red List Index (RLI) for amphibian species assessed in South Africa. The slope of the line indicates the rate at which species within each taxonomic group are becoming more threatened over time. The lower the value, the closer the group of species is towards extinction (if the value is 0, all species are extinct). Aggregate refers to the national average RLI across all assessed taxonomic groups."
...
::: {style="font-size:90%; color:#626262; margin-right: -10px; line-height: 1.7;"}
Before and after images showing a reduction in pine invasion from July 2021 (left) to April 2022 (right) at the type locality of <i>Arthroleptella rugosa</i> through the use of a controlled ecological burn. (© Alouis and Keir Lynch Bionerds)
:::
:::::::::::::::::
...
```{r}
#| label: fig-5
#| fig-cap: "Key pressures on South Africa’s amphibians, based on the percentage of impacted threatened and Near Threatened species, using the IUCN Threat Classification Scheme [@i2025a]."
#| fig-align: center
#| out.width: 100%
...
recovery_projects <- readxl::read_xlsx("./data/sp_am_table5.xlsx")%>%
                     mutate(`Get involved` = ifelse(is.na(`Get involved`), "", `Get involved`)) 
...
-   [Re:wild](https://www.rewild.org/)

-   [Synchronicity Earth](https://www.synchronicityearth.org/)

-   [Rainforest Trust](https://www.synchronicityearth.org/)
:::
...
```{r}
#| label: tbl-4
#| tbl-cap: List of contributors and reviewers to the assessment of South Africa amphibians and other activities related to amphibian conservation.   
#| classes: plain
