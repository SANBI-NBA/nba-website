...
title: "Freshwater Crabs" # Compulsory
...
# ---- Freshwater Crabs ----
df_fwfcrabs <- df %>%
  filter(`Taxonomic grouping` == "Freshwater Crabs") %>%
  collapse_rare() %>%
  mutate(Status = factor(Status, levels = status_levels))

df_fwfcrabs_en <- df %>%
  filter(`Taxonomic grouping` == "Freshwater Crabs", Endemism == "Endemic") %>%
  collapse_rare() %>%
  mutate(Status = factor(Status, levels = status_levels))

df_fwfcrabs_wide    <- make_wide(df_fwfcrabs, "Overall", all_levels)
df_fwfcrabs_en_wide <- make_wide(df_fwfcrabs_en, "Endemic", all_levels)

df_combined <- bind_rows(df_fwfcrabs_wide, df_fwfcrabs_en_wide)
...
dat1_sel$Taxon <- "Overall freshwater crabs"
dat2_sel$Taxon <- "Endemic freshwater crabs"
...
df_fwfcrabs <- df_rlis%>%
  filter(Taxon %in% c("Freshwater Crabs", "Aggregate"))
...
p <- nba_pressure_bar_plot(df, TAXON = "Freshwater Crabs")
p + scale_x_discrete(
  labels = function(x) {
