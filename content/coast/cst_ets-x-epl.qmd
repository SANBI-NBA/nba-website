...
unlucky <- read.csv("outputs/unlucky_table.csv") %>% rename(`Threat status`= Threat.status, `Protection level` = Protection.level, Types = mapcode)

table_final <- nba_tbl_comb(
  DF = unlucky,
  GROUP = Types,
  THR = `Threat status`,
  PRO = `Protection level`,
  FILE = "csv")
...
![High-risk coastal ecosystem types that are highly threatened and under-protected.](imgs/cst_unluckies_map_new2.png){#fig-unlucky}
