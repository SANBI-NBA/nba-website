...
Ecosystem threat status is one of the headline indicators of the National Biodiversity Assessment. It is an indicator of the degree to which ecosystems are still intact or, alternatively, losing vital aspects of their structure, function or composition.
...
For NBA 2025, the RLE framework was applied across all realms, and the results are summarised here for the 190 coastal ecosystem types that were assessed. For more details on ecosystem threat status, see the [general introduction](../integrated-findings/chl_rle_explained.qmd){target="_blank"} to this headline indicator.

## Coastal ecosystem threat status

The South African coast is proportionately more threatened than the rest of the country (entire non-coastal territory combined, @fig-etsgraph, @tbl-etsnumbers). In fact, the coast (`r percent_types(cet_thr_n, cet_n)`% threatened ecosystem types) proportionately has `r astext(c_nc_prop_thr_n)` times more threatened ecosystem types than the rest of the country (`r round(ncet_thr_n/ncet_n*100)`%), which is proportionately `r astext(c_nc_prop_thr_e)` times more by extent (coastal: `r percent_extent(ets, "thr", "coast")`%, non-coastal: `r round(nc_thr_e/nc_e*100)`%). Areas where threatened ecosystem types are concentrated are around the Orange River mouth, through most of the Western Cape, in St Francis and Algoa bays, and along virtually the entire East Coast (@fig-etsmap).
...
```{r}
#| label: tbl-ets
#| tbl-cap: "Threat status of coastal ecosystem types in South Africa. CR = Critically Endangered. EN = Endangered. VU = Vulnerable. NT = Near Threatened. LC = Least Concern."
...
cst <- read_csv(here("outputs/cst_results_for_integration.csv")) %>% 
  ...
  rename(Realm = realm, Subrealm = zone, `Ecosystem Type`=type, 
         `Threat Status` = RLE) %>% 
  mutate(`Threat Status` =  case_match(`Threat Status`,
